...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/08/23 11:49:09 by aelbasri          #+#    #+#             */
/*   Updated: 2024/08/23 11:49:12 by aelbasri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_intersections_horizontal(t_ray_cast *cast, t_ray \
*ray, t_data *data, double angle)
{
	while (cast->y_max - (cast->j * cast->y_init) >= 0)
	{
		ray->x = data->player->px + ((((double)(cast->y_init * \
		T_SIZE)) - data->player->py) / tan(angle));
		if (ray->x < 0)
			ray->x = 0;
...
void	get_intersections_vertical(t_ray_cast *cast, t_ray \
*ray, t_data *data, double angle)
{
	while (cast->x_max - (cast->x_init * cast->j) >= 0)
	{
		ray->y = data->player->py + ((((double)(cast->x_init * \
		T_SIZE)) - data->player->px) * tan(angle));
		if (ray->y < 0)
			ray->y = 0;
