...
t_ray    *ray_create(t_data *data, double angle)
{
    ...
    /* √((x1 - x)² + (y1 - y)²) < √((x1 - x)² + (y1 - y)²) */
	if (sqrt((pow(ray->x - data->player->px, 2) + pow((double)(cast.y_init * T_SIZE) - data->player->py, 2)))\
	< sqrt((pow(data->player->px - (double)(cast.x_init * T_SIZE), 2) + pow(data->player->py - ray->y, 2))))
    {
