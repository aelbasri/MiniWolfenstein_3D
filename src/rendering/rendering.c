...
void    one_wall_rendering(t_data *data, int j, t_ray *ray)
{
    ...
    while (render.i <= data->hi)
    {
        ...
        else if (render.i >= render.a && render.i <= render.a + render.wall_height)
        {
            ...
            render.color = *((unsigned int*)(data->walls[render.b].addr + ((int)((data->text_y * render.y) / render.wall_height) * \
            data->walls[render.b].line_length + (int)((data->text_x * render.x) / T_SIZE) * (data->walls[render.b].bits_per_pixel / 8))));
